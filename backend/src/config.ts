 ...
 */
function readContractId(env: NodeJS.ProcessEnv, canonical: string, legacy: string): string | undefined {
...
export function loadConfig(env: NodeJS.ProcessEnv = process.env): BackendConfig {
  const contractIds: ContractIds = {
    factory: readContractId(env, 'QUEUE_FACTORY_CONTRACT_ID', 'LINEPROOF_FACTORY_CONTRACT_ID'),
    queue: readContractId(env, 'QUEUE_CONTRACT_ID', 'LINEPROOF_QUEUE_CONTRACT_ID'),
    enrollment: readContractId(env, 'ENROLLMENT_CONTRACT_ID', 'LINEPROOF_ENROLLMENT_CONTRACT_ID'),
    identity: readContractId(env, 'IDENTITY_CONTRACT_ID', 'LINEPROOF_IDENTITY_CONTRACT_ID'),
    escrow: readContractId(env, 'ESCROW_CONTRACT_ID', 'LINEPROOF_ESCROW_CONTRACT_ID'),
  };

  const contractsConfigured = Object.values(contractIds).some((id) => Boolean(id));
